...
void PixmapCanvas::loadDir( QString dir_name, QString filter )
{
  max_width  = 0;
  max_height = 0;
  QDir d(dir_name);
...
void PixmapCanvas::resizeEvent( QResizeEvent *e )
{
  if( !isVisible() )
    return;
  setNumCols( width() / (max_width+4) );
...
PixLoaderDialog::~PixLoaderDialog()
{
  disconnect( ok );
  disconnect( cancel );
  disconnect( canvas );
...
int PixLoaderDialog::exec(QString filter)
{
  setResult( 0 );
...
void PixLoaderDialog::resizeEvent( QResizeEvent *e )
{
  int w = width();
  int h = height();
...
void PixLoaderDialog::filterChanged()
{
...
QPixmap PixmapLoader::selectPixmap(QString &name, QString filter)
{
  if( pix_dialog == NULL )
    pix_dialog = new PixLoaderDialog;
...
void PixmapLoader::setCaching( bool b )
{
  if( b == TRUE )
    {
      caching = TRUE;
