...
QList<KWMainWindow>* KWMainWindow::memberList = 0L;

KWMainWindow::KWMainWindow( const char *name )
    : QWidget( 0L, name )
{
    kmenubar = 0L;
    kmainwidget = 0L;
    ...
    if( !memberList )
      memberList = new QList<KWMainWindow>;
...
void KWMainWindow::closeEvent ( QCloseEvent *e){
...
bool KWMainWindow::queryExit(){
  return true;
}

int KWMainWindow::addToolBar( KToolBar *toolbar, int index )
{
  if ( index == -1 )
...
void KWMainWindow::setView( QWidget *view, bool show_frame )
{
  kmainwidget = view;
  if( show_frame ){
...
void KWMainWindow::focusInEvent( QFocusEvent *)
{
  repaint( FALSE );
}

void KWMainWindow::focusOutEvent( QFocusEvent *)
{
  repaint( FALSE );
}

void KWMainWindow::show ()
{
    QWidget::show();
    updateRects();
}

void KWMainWindow::updateRects()
{
    //debug ("Update");
    int t=0, b=0, l=0, r=0;
...
void KWMainWindow::saveYourself(){
  // Do session management (Matthias)
  if (kapp->topWidget() != this) return;
  KConfig *config = kapp->getSessionConfig();

//Jochen
  saveData(config);

  QListIterator<KWMainWindow> it(*memberList);
  int n = 0;
...
//Matthias
void KWMainWindow::savePropertiesInternal (KConfig* config, int number)
{
    QString entry;
    QStrList entryList;
...
//Matthias
bool KWMainWindow::readPropertiesInternal (KConfig* config, int number)
{
...
void KWMainWindow::setFrameBorderWidth(int size){
...
//Matthias
bool KWMainWindow::canBeRestored(int number){
  if (!kapp->isRestored())
    return False;
  KConfig *config = kapp->getSessionConfig();
  if (!config)
...
//Matthias
const QString KWMainWindow::classNameOfToplevel(int number){
  if (!kapp->isRestored())
    return "";
  KConfig *config = kapp->getSessionConfig();
  if (!config)
...
//Matthias
bool KWMainWindow::restore(int number){
...
// Matthias
void KWMainWindow::setUnsavedData( bool b){
  KWM::setUnsavedDataHint(winId(), b);
}


void KWMainWindow::resizeEvent( QResizeEvent *ev )
{
    if (kmainwidget && kmainwidget->minimumSize() == kmainwidget->maximumSize())
        return;
    if (kmainwidget && kmainwidget->minimumSize().height() == kmainwidget->maximumSize().height())
...
KStatusBar *KWMainWindow::statusBar()
{
  if (!kstatusbar)
...
KToolBar *KWMainWindow::toolBar( int ID )
{
  KToolBar* result = 0;
...
void KWMainWindow::enableToolBar( KToolBar::BarStatus stat, int ID )
{
...
void KWMainWindow::enableStatusBar( KStatusBar::BarStatus stat )
{
...
bool KWMainWindow::hasMenuBar(){
  return kmenubar != 0;
}

bool KWMainWindow::hasStatusBar(){
  return kstatusbar != 0;
}

bool KWMainWindow::hasToolBar( int ID = 0){
